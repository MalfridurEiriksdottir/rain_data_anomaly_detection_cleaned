...
def create_plots_with_error_markers(
    ...
    Plots:
      - Raw radar data (VeVa),
      - Raw gauge data,
      - 24h adjusted radar (network-adjusted from nearest gauges),
      - Final (regional) adjusted rainfall (red line),
    ...
    for coord in tqdm(all_data.keys(), desc="Generating Plots"):
        df_final = all_data[coord]  # Final data for sensor
        fig = go.Figure()
        ...
        # --- Plot 24h Adjusted Radar (local adjustment) ---
        if 'Batch_Alpha_Adjusted_Radar' in df_final:
            fig.add_trace(go.Scatter(x=df_final.index, y=df_final['Batch_Alpha_Adjusted_Radar'], mode='lines',
                                     name='24h Adjusted Radar based on nearest 5 gauges',
                                     line=dict(color='purple', dash='dashdot', width=1.5)))
        else:
            print(f"Warning: 'Batch_Alpha_Adjusted_Radar' missing for {coord}")

        # --- Plot Final Adjusted Rainfall (regional adjustment) in Red ---
        if 'Final_Adjusted_Rainfall' in df_final:
            fig.add_trace(go.Scatter(x=df_final.index, y=df_final['Final_Adjusted_Rainfall'], mode='lines',
                                     name='Final Adjusted (Regional)', 
                                     line=dict(color='red', width=2)))
        else:
            print(f"Warning: 'Final_Adjusted_Rainfall' missing for {coord}")

        if 'Radar_Freg_Adjusted' in df_final:
            fig.add_trace(go.Scatter(x=df_final.index, y=df_final['Radar_Freg_Adjusted'], mode='lines',
                                     name='Raw Radar * f_reg', # Specific name
                                     line=dict(color='green', width=2))) # Orange dash
        else:
            # This column *should* exist if the main script ran correctly
            logger.warning(f"'Radar_Freg_Adjusted' missing for plotting {coord}")


        print('Checking for batch boundaries flags')
        # plot the batch flagging
        if 'Batch_Flag' in df_final.columns:
            print('Batch_Flag found in df_final')
            # Create a boolean Series for the faulty periods
            flag_series = df_final['Batch_Flag'].fillna(False).astype(bool)
            if flag_series.any():
                print('Faulty gauge intervals found')
                # Find start and end times for contiguous flagged intervals.
                # Compute the difference between successive values to detect transitions.
                flag_diff = flag_series.astype(int).diff().fillna(0)
                # Start when diff==1 and end when diff==-1.
                start_times = flag_series.index[flag_diff == 1]
                end_times = flag_series.index[flag_diff == -1]
                # If the first entry is True, then the interval starts at the beginning.
                if flag_series.iloc[0]:
                    start_times = start_times.insert(0, flag_series.index[0])
                # If the last entry is True, then the interval runs until the last timestamp.
                if flag_series.iloc[-1]:
                    end_times = end_times.append(pd.Index([flag_series.index[-1]]))
                # Overlay vrect for each flagged interval.
                for s, e in zip(start_times, end_times):
                    # print('Adding vertical rectangle for faulty gauge from', s, 'to', e)
                    fig.add_vrect(x0=s, x1=e, fillcolor="red", opacity=0.3, line_width=0,
                                  annotation_text="Faulty Gauge", annotation_position="top left")
        ...
        if not sensor_events.empty:
            for _, event in sensor_events.iterrows():
                if pd.notna(event['event_start']) and pd.notna(event['event_end']):
                    ...
                    print(f"Warning: Invalid event times for {coord}.")
...
def plot_flagging_metrics(df: pd.DataFrame, coord_name: str = "", events_df: pd.DataFrame = None) -> go.Figure:
    # print('Inside plot_flagging_metrics function')
    """
    Create a multi-panel Plotly figure showing:
      1) Data & Flags
      2) Rolling Absolute Error (60min)
      3) Rolling Proportion Flagged (60min)
      4) Adjusted Diff from Net
      5) Adjusted Ratio from Net
      6) Neighbor Count
      7) Gauge=0 Condition
    Also includes event shading and highlights faulty gauge intervals if present.
    """
    import plotly.graph_objects as go
    import plotly.subplots as sp
    from config import CFG

    # Define subplot titles to reflect the new metrics
    subplot_titles = (
        "1. Data, Flags & Adjustments",       # row 1
        "2. Rolling Abs Error (60min)",       # row 2
        "3. Rolling Prop Flagged (60min)",    # row 3
        "4. Adj. Diff From Net",              # row 4
        "5. Adj. Ratio From Net",             # row 5
        "6. Neighbor Count",                  # row 6
        "7. Gauge=0 Condition"                # row 7
    )
    ...
    # ─────────────────────────────────────────────────────────────────────────────
    print('Adding data, flags & adjustments to row 1')
    if 'Radar_Data_mm_per_min' in df:
        fig.add_trace(go.Scatter(
            x=df.index, y=df['Radar_Data_mm_per_min'],
            mode='lines', name='Raw Radar', line=dict(color='orange', width=1)
        ), row=1, col=1)

    if 'Gauge_Data_mm_per_min' in df:
        fig.add_trace(go.Scatter(
            x=df.index, y=df['Gauge_Data_mm_per_min'],
            mode='lines', name='Gauge', line=dict(color='blue', width=1)
        ), row=1, col=1)

    if 'Batch_Alpha_Adjusted_Radar' in df:
        fig.add_trace(go.Scatter(
            x=df.index, y=df['Batch_Alpha_Adjusted_Radar'],
            mode='lines', name='24h Adjusted Radar',
            line=dict(color='purple', dash='dashdot', width=1.5)
        ), row=1, col=1)

    if 'Final_Adjusted_Rainfall' in df:
        fig.add_trace(go.Scatter(
            x=df.index, y=df['Final_Adjusted_Rainfall'],
            mode='lines', name='Final Adjusted (Regional)',
            line=dict(color='red', width=2)
        ), row=1, col=1)

    # Plot flagged anomalies in row 1
    if 'Flagged' in df and df['Flagged'].any():
        flagged_points = df['Gauge_Data_mm_per_min'].where(df['Flagged'])
        if not flagged_points.isnull().all():
            fig.add_trace(go.Scatter(
                x=flagged_points.index, y=flagged_points,
                mode='markers', name='Anomalies',
                marker=dict(color='red', size=6, symbol='x')
            ), row=1, col=1)

        # Add vertical lines to indicate 24h batch boundaries
    if not df.index.empty:
        print('Adding vertical lines for batch boundaries')
        # Assume the batch boundaries align with multiples of 24 hours from sensor start
        sensor_start = df.index.min()
        sensor_end = df.index.max()
        batch_boundaries = pd.date_range(start=sensor_start, end=sensor_end, freq='24H')
        for boundary in batch_boundaries:
            fig.add_vline(x=boundary, line=dict(color='black', dash='dash', width=1), opacity=0.5)
    print('Checking Batch_Flag in df columns')
    # Shading for faulty gauge intervals (Batch_Flag)
    if 'Batch_Flag' in df.columns:
        flag_series = df['Batch_Flag'].fillna(False).astype(bool)
        print('Batch_Flag found in df')
        if flag_series.any():
            print('Faulty gauge intervals found')
            flag_diff = flag_series.astype(int).diff().fillna(0)
            start_times = list(df.index[flag_diff == 1])
            end_times = list(df.index[flag_diff == -1])
            if flag_series.iloc[0]:
                print('Adding start time for faulty gauge with coordinate:', coord_name, 'and start time:', df.index[0])
                start_times.insert(0, df.index[0])
            if flag_series.iloc[-1]:
            ...
            for s, e in zip(start_times, end_times):
                ...
                fig.add_vrect(
                    ...
                    annotation_text="Faulty Gauge",
                    annotation_position="top left",
                    row='all', col=1
                )

    # ─────────────────────────────────────────────────────────────────────────────
    # Row 2: Rolling Absolute Error (60min)
    # ─────────────────────────────────────────────────────────────────────────────
    if 'Rolling_Abs_Error' in df:
        fig.add_trace(go.Scatter(
            x=df.index, y=df['Rolling_Abs_Error'],
            mode='lines', name='Rolling Abs Error (60min)',
            line=dict(color='magenta', width=1)
        ), row=2, col=1)
        # Add threshold line
        fig.add_hline(
            y=CFG.ABS_DIFF_THRESHOLD_MM_MIN,
            line=dict(color='red', dash='dash', width=1),
            row=2, col=1
        )

    # ─────────────────────────────────────────────────────────────────────────────
    # Row 3: Rolling Proportion Flagged (60min)
    # ─────────────────────────────────────────────────────────────────────────────
    if 'Rolling_Prop_Flagged' in df:
        # print('Rolling_Prop_Flagged found in df')
        fig.add_trace(go.Scatter(
            x=df.index, y=df['Rolling_Prop_Flagged'],
            mode='lines', name='Rolling Prop Flagged (60min)',
            line=dict(color='darkgreen', width=1)
        ), row=3, col=1)
        # Add 50% line
        fig.add_hline(y=0.7, line=dict(color='red', dash='dash', width=1),
                      row=3, col=1)
    else:
        print('Rolling_Prop_Flagged not found in df')

    # ─────────────────────────────────────────────────────────────────────────────
    # Row 4: Adjusted Diff from Net
    # ─────────────────────────────────────────────────────────────────────────────
    if 'Adjusted_Diff_from_network' in df:
        fig.add_trace(go.Scatter(
            x=df.index, y=df['Adjusted_Diff_from_network'],
            mode='lines', name='Adj. Diff from Net', line=dict(color='purple')
        ), row=4, col=1)
    fig.add_hline(
        y=CFG.ABS_DIFF_THRESHOLD_MM_MIN, line=dict(color='red', dash='dash', width=1),
        row=4, col=1
    )
    fig.add_hline(
        y=-CFG.ABS_DIFF_THRESHOLD_MM_MIN, line=dict(color='red', dash='dash', width=1),
    ...
    # ─────────────────────────────────────────────────────────────────────────────
    # Row 5: Adjusted Ratio from Net
    # ─────────────────────────────────────────────────────────────────────────────
    if 'Adjusted_Ratio_From_Network' in df:
        fig.add_trace(go.Scatter(
            x=df.index, y=df['Adjusted_Ratio_From_Network'],
            mode='lines', name='Adj. Ratio from Net', line=dict(color='green')
        ), row=5, col=1)
    fig.add_hline(
        y=1 + CFG.RATIO_THRESHOLD, line=dict(color='red', dash='dash', width=1),
        row=5, col=1
    )
    fig.add_hline(
        y=1 - CFG.RATIO_THRESHOLD, line=dict(color='red', dash='dash', width=1),
    ...
    fig.add_hline(
        y=1, line=dict(color='grey', dash='dot', width=1),
        row=5, col=1
    )

    fig.update_layout(
        height=1800,
        title_text=f"Flagging Metrics for {coord_name}",
        template=CFG.PLOT_TEMPLATE,
        showlegend=True,
        hovermode="x unified"
    )

    # Optionally add event shading to *all* rows
    if events_df is not None and not events_df.empty:
